...
where ResellerKey in ( 	select ResellerKey
						from adventureworksdw2019.factresellersales
						group by ResellerKey
						having count(distinct SalesOrderNumber) > 10
					)*/

/*select ResellerKey, count(*) as totOrdiniByReseller 
from (
		select ResellerKey, SalesOrderNumber
        from adventureworksdw2019.factresellersales
        group by ResellerKey, SalesOrderNumber
) as ResellerKeyByOrder
group by ResellerKey
having count(*) > 10;*/

/*select ResellerKey, count(distinct SalesOrderNumber) as countOrder
						from adventureworksdw2019.factresellersales
						group by ResellerKey
						having count(distinct SalesOrderNumber) > 10*/
